...
double evalAverageError(const Storage& q_out, const Storage& ref_q);
...
class LandingOptimizationSystem : public OptimizerSystem {
   public:
    ...
    void evalSimulation( const Vector &newParameters, double &f ) const {
        ...
        double error = evalAverageError(q_out, ref_q);
        //cout << "error = " << rms_error << endl;
...
//	try {
        ...
        Vec3 platform_origin(0.0, 0.015, 0.35);
        ...
        double error = evalAverageError(states, *q_desired);
        fwdLog << "kinematic error = " << error;

        int pause;
        cout << "finished and closing...";
        //cin >> pause;
...
double evalAverageError(const Storage& q_out, const Storage& ref_q)
{
    ...
    double diff1 = 0.0;
    double diff2 = 0.0;
    Vector refdata1 = Vector(refSize);
    Vector outdata1 = Vector(outSize);
    Vector refdata2 = Vector(refSize);
    Vector outdata2 = Vector(outSize);
    cout << time_stamps.getSize() << " time steps" << endl;
    
    // Take trapezoidal integral of error
    for (int i = 0; i < time_stamps.getSize() - 1; i++)
    {
        t1 = time_stamps.get(i);
        t2 = time_stamps.get(i + 1);
        ref_q.getDataAtTime(t1, refSize, refdata1);
        q_out.getDataAtTime(t1, outSize, outdata1);

        ref_q.getDataAtTime(t2, refSize, refdata2);
        q_out.getDataAtTime(t2, outSize, outdata2);

        for (int j = 1; j < refSize; j++)
        {
            string coord = refCoordinates.get(j);
            diff1 = refdata1(j - 1) - outdata1(out_indices.get(j - 1));
            diff2 = refdata2(j - 1) - outdata2(out_indices.get(j - 1));

            // apply the trapezoidal quadriture development
            average_error += ( sqrt(diff1*diff1) + sqrt(diff2*diff2) )*(t2-t1)/2;
        }
    }

    average_error = average_error / (tf - ti);
